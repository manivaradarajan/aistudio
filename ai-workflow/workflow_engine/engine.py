...
def _run_steps(
    ...
    for step in steps:
        ...
        if step_type == "extract_text_from_pdf":
            current_input_path = handlers.handle_extract_text_step(
                merged_step,
                config,
                prefix,
                current_input_path,
            ...
            current_input_path = handlers.handle_convert_script_step(
                merged_step, prefix, current_input_path, force_regeneration
            )
        elif step_type == "chat":
            current_input_path = handlers.handle_chat_step(
                merged_step,
                config,
                prefix,
                current_input_path,
...
def run_workflow(
    config: Config, prefix: str, source_path: Path, force_regeneration: bool
):
    """
...
def _generate_page_ranges(
    page_ranges_config: Union[List, Dict], source_pdf_path: Path
) -> List[str]:
    ...
    if isinstance(page_ranges_config, dict) and "pageset_size" in page_ranges_config:
        ...
        logging.info(
            f"Detected 'pageset_size' of {pageset_size}. Generating page ranges dynamically."
        )
        ...
        try:
            ...
            raise ValueError(
                f"Failed to read source PDF '{source_pdf_path}' to determine total pages for pageset generation. Error: {e}"
            )
        ...
        logging.info(
            f"Processing from page {start_page_overall} to {end_page_overall}."
        )
        ...
        logging.info(
            f"Generated {len(page_ranges_list)} page ranges: {page_ranges_list[:5]}..."
        )
        return page_ranges_list

    raise TypeError(
        f"Unsupported type for 'page_ranges': {type(page_ranges_config)}. Must be a list of ranges or a dict with 'pageset_size'."
    )


def _process_page_ranges(
    config: Config, source_pdf_path: Path, force_regeneration: bool
):
    """
    ...
    for item in tqdm(page_ranges, desc="Processing page ranges"):
        ...
        # --- FIX START: This block was incorrectly indented inside the 'else' statement ---
        prefix = (
            f"{source_pdf_path.stem}-{page_range_str}-{custom_suffix}"
            if custom_suffix
            else f"{source_pdf_path.stem}-{page_range_str}"
        )
        logging.info(
