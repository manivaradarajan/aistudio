...
- **Progressive Disclosure:** The user is never overwhelmed. They are first presented with the primary text. Deeper information, like commentary, is revealed only upon explicit interaction (a click). Similarly, the full table of contents is neatly organized into collapsible sections.

- **Context is Key:** The design ensures the user never loses their place.

  - **On desktop**, the three-pane layout keeps the navigator, main text, and commentary visible simultaneously, allowing for cross-referencing and study.
  - **On mobile**, overlays are used instead of separate pages, so the user always feels grounded in the main text they are reading.

- **Responsive Duality:** The application serves two distinct use cases based on the device:
  - **Desktop as a Workbench:** The spacious, multi-pane layout is optimized for research, comparison, and deep study.
  - **Mobile as a Reader:** The single-pane, focused layout with easy navigation (arrows, bottom sheet) is optimized for a linear, immersive reading experience.
...
1.  **Data Layer (The "Brain"):**
    ...
    - **Source of Truth:** All textual content is stored in structured, generic JSON files. A root `texts.json` manifest file lists all available texts.
...
3.  **Presentation Layer (The "Look" - CSS):**
    ...
    - **Responsive Breakpoints:** A single, clear media query (`@media (max-width: 800px)`) cleanly separates the desktop and mobile styles, completely transforming the layout as needed.
...
4.  **Behavior Layer (The "Action" - JavaScript):**
    ...
    - **Rendering Engine:** The `handleRouteChange` function acts as a central controller that parses the URL, loads the correct data, and calls generic rendering functions (`renderNavigator`, `loadSection`) to build the DOM. This creates a predictable, unidirectional data flow.
...
1.  **The Schema is the Contract:** All code will be written to conform to the generic JSON schema. To support a new text, you modify the **data**, not the application code.

2.  **The URL is State:** The application is stateless. Any view can be perfectly reconstructed just from the URL. All navigation actions must result in a URL hash change.

3.  **Generic Before Specific:** Functions will continue to be written generically. Instead of `loadAnuvaka`, we have `loadSection`. Instead of hard-coded DOM classes, we use generic ones like `.item-container`. This is the key to scalability.
